...
import com.qanonTesting.adminMenu.Feeds;
...
//			CommonMethods2.closePopUp();	//Close pop up if exists/opened
			...
			WriteTestResult.addRowInDetailsSheet(pageName +" SCREEN", "ENTER SCREEN", pageName, CommonMethods2.GetCurrentTimeStamp(), CommonValues.environmentName, CommonValues.tenantName, CommonValues.subTenantName, CommonValues.txtUsername2, "", time, message);
		}
		catch(Exception ex)
		{
...
//			CommonMethods2.closePopUp();	//Close pop up if exists/opened
			...
			WriteTestResult.addRowInDetailsSheet(pageName +" SCREEN", "ENTER SCREEN", pageName, CommonMethods2.GetCurrentTimeStamp(), CommonValues.environmentName, CommonValues.tenantName, CommonValues.subTenantName, CommonValues.txtUsername2, "", responsetime, message);
		}
		catch(Exception ex)
		{
			CreateLogs.writeLogs(ex.getMessage());
	...
	public static void verifyIsPageExists(String searchPath, String nameOfPage, String nameOfModule) throws Exception
	{
		...
		else
		{
			...
			DeclareVariables.failResultMessage = "'" +nameOfPage +"' PAGE NOT EXISTS FOR '" +CommonValues.txtUsername2 +"'";
			CreateLogs.writeLogs(DeclareVariables.failResultMessage);
			//Print Result in Excel File
			WriteTestResult.addRowInBasicSheet("", "FAIL", "", CommonValues.dateTime, CommonValues.environmentName, CommonValues.subTenantName, nameOfModule, CommonValues.txtUsername2, DeclareVariables.failResultMessage);
			WriteTestResult.addRowInDetailsSheet("VERIFY "+nameOfPage, nameOfPage +" SCREEN OPEN", nameOfPage, CommonMethods2.GetCurrentTimeStamp(), CommonValues.environmentName, CommonValues.tenantName, CommonValues.subTenantName, CommonValues.txtUsername2, "", "", DeclareVariables.failResultMessage);
		}
	}	
...
//        	Thread.sleep(1500);
        ...
        else
        {
        	...
        	String errorMsgPath = "//div[@class='ovflw-hdn has-trans-300']//div[@class='rslts-wg-itm-rslt rslts-wg-itm-el inln-blck v-mid-algn cursor-pntr']";
        	
        	if(DeclareVariables.driver2.findElements(By.xpath(errorMsgPath)).size()>0)
        	{
        		DeclareVariables.savePopUpMsg = FunctionLibraries2.locator("xpath", errorMsgPath).getText();
            	CreateLogs.writeLogs("ERROR MESSAGE FOUND ::- " +DeclareVariables.savePopUpMsg);
            	Thread.sleep(2000);
            	robot.keyRelease(KeyEvent.VK_ESCAPE);
            	Thread.sleep(5000);           	
            	Feeds.fillProfileDetails();
...
//    			CreateLogs.writeLogs("TIME WHEN HIT 'UPLOAD ATTACHMENT' BUTTON ON 'FEEDS' PAGE:- ");// +t1 +" milliseconds");
        	...
        	{
        		CreateLogs.writeLogs("NO SAVE POP UP AND NO ERROR MESSAGE FOUND. PLEASE CHECK");
        	}        	
        }
...
//        	System.out.println("FINAL TIME ::- " +DeclareVariables.timeTaken);
        	WriteTestResult.addRowInDetailsSheet("SAVE FEED", "SAVE FEED", "FEEDS", CommonMethods2.GetCurrentTimeStamp(), CommonValues.environmentName, CommonValues.tenantName, CommonValues.subTenantName, CommonValues.txtUsername2, "", DeclareVariables.timeTaken, DeclareVariables.passResultMessage);
    ...
    //Method to click Upload Attachment and open window popup 
    public static void clickUploadAttachment() throws IOException
    {
    	String adminUploadBtnXPath = "//div[@class='VZ-WGT-BDY-WRP VZ-WGT-BDY-WRP-QONCIERGE']//div[@class='vz-toolbar-trg-lbl inln-blck v-top-algn'][contains(text(),'UPLOAD ATTACHMENT')]";
    	
    	//Click Image Upload			
        WebElement adminUploadFeedFile = FunctionLibraries2.locator("xpath", adminUploadBtnXPath);
        if(adminUploadFeedFile!= null)
        {
        	DeclareVariables.driver2.findElement(By.xpath(adminUploadBtnXPath)).click();
        	WriteTestResult.addRowInDetailsSheet("LOAD FILE ATTACHMENT", "UPLOAD ATTACHMENT CLICKED", "FEEDS", CommonMethods2.GetCurrentTimeStamp(), CommonValues.environmentName, CommonValues.tenantName, CommonValues.subTenantName, CommonValues.txtUsername2, "", "40 milliseconds", "UPLOAD WINDOW OPEN");
        }
        else
        {
        	WriteTestResult.addRowInDetailsSheet("LOAD FILE ATTACHMENT", "UPLOAD ATTACHMENT", "FEEDS", CommonMethods2.GetCurrentTimeStamp(), CommonValues.environmentName, CommonValues.tenantName, CommonValues.subTenantName, CommonValues.txtUsername2, "", "", "UPLOAD ATTACHMENT NOT FOUND");
        }
    }
